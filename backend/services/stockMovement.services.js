...
export const getItemStockLocationsService = async (itemId) => {
  if (!mongoose.Types.ObjectId.isValid(itemId)) {
    throw new ApiError(HTTP_STATUS.BAD_REQUEST, "Invalid itemId");
  }
  ...
  const locationIds = latestMovements.map((movement) => movement._id);
  const locations = await Location.find({
    _id: { $in: locationIds },
    status: "active",
  })
    .select("name locationNo status")
    .lean();
  ...
  const mappedLocations = latestMovements
    .map((movement) => {
      const location = locationMap.get(String(movement._id));
      if (!location) {
        return null;
      }
  ...
  if (latestMovements.length === 0 && item.location) {
